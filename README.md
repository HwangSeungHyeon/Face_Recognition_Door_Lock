...
## 프로젝트 계획 이유
  ...
  * 라즈베리 파이에서 모든 작업을 처리하기 때문에 다양한 공간에서 적용할 수 있다.
...
## 사용 방법
  ...
  ## 1. 사용자 등록 https://youtu.be/Ubvjq_VEEco
      ...
      1-2. 사람 이름을 입력하면, 해당 이름을 가진 폴더가 생성된다.
      1-3. 라즈베리 파이 카메라 모듈 v2로 사진을 촬영하기 전 5초의 유예 기간이 주어진다.
      1-4. 0.5초의 간격으로 10장의 사진을 촬영한다.
      1-5. 생성한 폴더에 사진을 저장한다.
      1-6. 촬영한 사진에서 얼굴 영역 검출한다.
      1-7a. 검출한 영역에 얼굴이 있을 경우 특징점 벡터를 추출한다.
      1-7b. 검출한 영역에 얼굴이 없을 경우 PASS
      1-8a. 얼굴이 하나라도 존재할 경우 생성한 데이터로 "사용자 이름".pkl을 생성한다.
      1-8b. 모든 사진에서 얼굴이 검출되지 않았을 경우 pkl 파일을 만들지 않는다.
